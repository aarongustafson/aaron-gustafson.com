...
#
# Copyright (c) 2015 Aaron Gustafson, http://aaron-gustafson.com/
...
module Linkblog
    ...
    def generate(site)
      if Pager.pagination_enabled?(site)
        if template = self.class.template_page(site)
          ...
          Jekyll.logger.warn "Linkblog Pagination:", "Pagination is enabled, but I couldn't find " +
          "an index.html page to use as the pagination template. Skipping pagination."
        end
      end
    end

    # Paginates the linkblog's posts. Renders the index.html file into paginated
  ...
  class Pager
    ...
    def self.pagination_candidate?(config, page)
      ...
      paginate_path = remove_leading_slash(config['linkblog_paginate_path'])
    ...
    def self.paginate_path(site, num_page)
      ...
      format = site.config['linkblog_paginate_path']
