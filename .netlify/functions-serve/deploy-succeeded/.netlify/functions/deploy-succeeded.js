		...
		function parse(options) {
			...
			if (!isBinaryRequest) {
				if (options.mediaType.format) {
					headers2.accept = headers2.accept
						.split(/,/)
						.map((preview) =>
							preview.replace(
				...
				if (options.mediaType.previews.length) {
					const previewsFromAcceptHeader =
						headers2.accept.match(/[\w-]+(?=-preview)/g) || [];
					headers2.accept = previewsFromAcceptHeader
	...
	"node_modules/@octokit/request/dist-node/index.js"(exports2) {
		...
		function fetchWrapper(requestOptions) {
			...
			const fetch =
				...
				globalThis.fetch /* istanbul ignore next */ ||
				nodeFetch;
			return fetch(
				requestOptions.url,
				Object.assign(
				...
				.then(async (response) => {
					...
					if ("deprecation" in headers2) {
						const matches =
							headers2.link &&
							headers2.link.match(/<([^>]+)>; rel="deprecation"/);
...
var require_dist_node10 = __commonJS({
	"node_modules/@octokit/plugin-paginate-rest/dist-node/index.js"(exports2) {
		...
		function iterator(octokit2, route, parameters) {
			...
			const requestMethod =
				typeof route === "function" ? route : octokit2.request;
			...
			for (const [scope, endpoints] of Object.entries(endpointsMap)) {
				for (const [methodName, endpoint] of Object.entries(endpoints)) {
					...
					scopeMethods[methodName] =
						octokit2.request.defaults(endpointDefaults);
				}
			}
			return newMethods;
		}
...
var require_dist_node12 = __commonJS({
	"node_modules/@octokit/rest/dist-node/index.js"(exports2) {
		...
		var VERSION = "19.0.7";
		var Octokit2 = core.Octokit.plugin(
