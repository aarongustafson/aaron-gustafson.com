...
try {
	...
	console.log(
		`>>> Image cache loaded: ${Object.keys(imageCache).length} entries`,
	);
	...
	if (originalCount !== cleanedCount) {
		console.log(
			`>>> Cleaned ${originalCount - cleanedCount} orphaned cache entries`,
		);
	}
} catch (err) {
			...
			.on("end", () => {
				...
				const hitRate =
					cacheHits + cacheMisses > 0
						...
						: "0.0";
				console.log(
					`>>> Images: ${processedCount} processed, ${skippedCount} skipped (${duration}ms, ${hitRate}% cache hit rate)`,
				);
			})
	);
};
