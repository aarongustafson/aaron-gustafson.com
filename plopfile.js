...
export default function (plop) {
  ...
  plop.setActionType("publishDraft", async function (answers, config, _plop) {
    ...
    const targetDir = config.targetDir || "posts";
    ...
    // Handle filename: check if draft already has a date prefix
    // Pattern: YYYY-MM-DD-slug.md
    const datePattern = /^(\d{4}-\d{2}-\d{2})-(.+)$/;
    const match = answers.draft.match(datePattern);
    let newFilename;

    if (match) {
      // Replace the existing date with today's date
      newFilename = `${datePrefix}-${match[2]}`;
    } else {
      // No existing date, prepend today's date
      newFilename = `${datePrefix}-${answers.draft}`;
    }

    const publishPath = path.join(__dirname, `src/${targetDir}`, newFilename);
    ...
    console.log(`✓ Published ${answers.draft} to ${targetDir}/${newFilename}`);
  ...
  plop.setGenerator("publish:post", {
    description: "Publish a draft as a post",
    prompts: [
      {
        type: "list",
        name: "draft",
        message: "Which draft would you like to publish as a post?",
  ...
  plop.setGenerator("publish:link", {
    description: "Publish a draft as a link",
    prompts: [
      {
        type: "list",
        name: "draft",
        message: "Which draft would you like to publish as a link?",
        choices: helpers.getDrafts(),
      },
    ],
    actions: [
