...
class PostSyndicator extends SocialMediaAPI {
  ...
  async syndicatePost(post) {
    ...
    const socialText = post.social_text || ContentProcessor.stripHtml(post.content_html);
    const linkedInContent = ContentProcessor.processContentForLinkedIn(post.content_html);
    ...
    try {
      ...
      const mastodonText = ContentProcessor.truncateText(`${socialText} ${post.url}`, 450);
    ...
    try {
      ...
      const bufferText = ContentProcessor.truncateText(`${socialText} ${post.url}`, 260);
