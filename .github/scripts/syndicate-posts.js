...
class PostSyndicator extends SocialMediaAPI {
  ...
  async syndicatePost(post) {
    ...
    // For LinkedIn posts, prefer an explicit excerpt when available, then
    // fall back to the feed summary or the full content.
    const linkedInSource =
      post.excerpt || post.summary || post.content_html;
