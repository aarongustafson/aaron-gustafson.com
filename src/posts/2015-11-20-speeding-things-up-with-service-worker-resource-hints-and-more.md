...
self.addEventListener("fetch", function (event) {
  ...
  if (re_jpg_or_png.test(request.url) && url_object.origin == location.origin) {
    ...
    if (supports_webp) {
      ...
      event.respondWith(fetch(webp_url, { mode: "no-cors" }));
    }
  }
});
```
...
gulp.task(
  "scripts",
  folder(source_folder, function (folder) {
    return gulp
      .src(path.join(source_folder, folder, "*.js"))
...
Service Workers are still pretty new (and [modestly supported](http://caniuse.com/#feat=serviceworkers)), but it’s definitely interesting to see what’s possible using them. [Like Jeremy](https://adactio.com/journal/9844), I want to do a bit more exploration into caching and how it may actually _increase_ the monetary cost of accessing a website if not used properly. Like any powerful tool, we need to wield it wisely.
