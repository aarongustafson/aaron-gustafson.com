...
Accessibility becomes even more of a challenge when you begin to factor in the dependency chain. Blind and low-vision users, for example, often use screen readers in order to access the Web. With screen readers, we need to consider the capabilities of both the browser _and_ the screen reader being used. If either piece of software isn’t up to the task or contains a bug our users suffer for it.
