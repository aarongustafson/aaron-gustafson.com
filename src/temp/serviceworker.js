			...
			event.respondWith(
				...
				caches.match(request).then((cached_result) => {
					if (cached_result) {
						// Update the cache in the background, but only if we’re not trying to save data
						if (!save_data && !slow_connection) {
							event.waitUntil(refreshCachedCopy(request, sw_caches.pages.name));
						}
						return cached_result;
					}
...
self.addEventListener("install", function (event) {
	...
	event.waitUntil(
		caches.open(sw_caches.static.name).then(function (cache) {
...
function respondWithFallbackImage(url, fallback = fallback_image) {
	...
	return caches.match(image).catch(respondWithServerOffline);
}
