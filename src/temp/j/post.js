...
(function (document) {
	...
	if (q) {
		...
		while (q--) {
			...
			if ($previous_heading) {
				$current_sibling =
					$previous_heading.nextElementSibling.nextElementSibling;
				// no blockquotes next to blockquotes
				while ($current_sibling.nodeName.toLowerCase() == "blockquote") {
					$current_sibling =
						$current_sibling.nextElementSibling.nextElementSibling;
				}
			}
...
(function (window) {
	if ("serviceWorker" in navigator) {
		...
		caches.match(location).then((item) => {
			if (item) {
		...
		$button.addEventListener(
			...
			function (e) {
				...
				caches.open(post_cache).then((cache) => {
					cache
						.add(location)
						.then(function () {
