...
async function registerPeriodicSync(widget) {
	console.log(
		"should we register a periodic sync?",
...
async function updateWidgets(host_id) {
	...
	const config = host_id ? { hostId: host_id } : { installed: true };

	let queue = [];
	await widgets.matchAll(config).then(async (widgetList) => {
...
async function initAllWidgets(widget) {
	...
	for (widget of all_widgets) {
		console.log(`Updating ${widget.definition.tag} with payload`, {
			template,
			data,
		});
...
self.addEventListener("activate", (event) => {
	...
	event.waitUntil(initAllWidgets());
});

self.addEventListener("widgetinstall", (event) => {
	console.log(`Installing ${event.widget.tag}`);
	event.waitUntil(initializeWidget(event.widget));
});

self.addEventListener("widgetuninstall", (event) => {
	console.log(`Uninstalling ${event.widget.tag}`);
	event.waitUntil(uninstallWidget(event.widget));
});
