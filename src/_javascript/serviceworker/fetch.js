self.addEventListener("fetch", (event) => {
	...
	// JSON & such
	if (/\.json$/.test(url) || /jsonp\=/.test(url)) {
		event.respondWith(
			caches.match(request).then((cached_result) => {
				...
				if (cached_result) {
					...
					if (!save_data && !slow_connection) {
						event.waitUntil(refreshCachedCopy(request, sw_caches.other.name));
					}
				...
				return (
					fetch(request)
						.then((response) => {
							...
							event.waitUntil(saveToCache(sw_caches.other.name, request, copy));
							return response;
						})
	...
	// HTML
			...
			event.respondWith(
				caches.match(request).then((cached_result) => {
					...
					if (cached_result) {
						// Update the cache in the background, but only if we’re not trying to save data
						if (!save_data && !slow_connection) {
							event.waitUntil(refreshCachedCopy(request, sw_caches.posts.name));
						}
						return cached_result;
					}
					// fallback to network
		...
		// all other pages - check the network &  cache response, then cache, offline fallback
		else {
			event.respondWith(
				// network first, then cache the result
				fetch(request)
					.then((response) => {
						const copy = response.clone();
						event.waitUntil(saveToCache(sw_caches.pages.name, request, copy)); // end waitUntil
						return response;
					})
					// fallback to offline page
					.catch(() => {
			...
			caches.match(request).then((cached_result) => {
				...
				// all others
					...
					// normal operation
						...
						return (
							fetch(request, fetch_config.images)
								.then((response) => {
									const copy = response.clone();
									event.waitUntil(
