...
(function (window, document) {
	...
	// Public Properties
	//
	WebmentionIO.existing_webmentions = [];

	//
	// Public Methods
	//

	WebmentionIO.processWebmentions = function (data) {
	...
	function collectExistingWebmentions() {
		...
		while (e--) {
			WebmentionIO.existing_webmentions.push(
				$existing_webmentions[e].getAttribute("id").replace("webmention-", ""),
			);
		}
	...
	function unhide($webmention_list) {
		// is the container hidden?
		if ($webmention_list.parentNode.hidden) {
			// individual container
			$webmention_list.parentNode.hidden = false;
			// whole section
			$webmention_list.parentNode.parentNode.hidden = false;
		}
	}
	...
	function identifyWebmentionCollections() {
		...
		while (w--) {
			...
			$webmention_list =
				$webmention_collection.querySelector(".webmentions__list");

			// Assignments
			$webmention_collection.type =
				$webmention_list.dataset.webmentionCollection;
			$webmention_collection.template =
				$webmention_list.dataset.webmentionTemplate;
			$webmention_collection.class =
				$webmention_list.dataset.webmentionTemplateClass;

			// Set up receivers
			types = $webmention_list.dataset.webmentionTypes.split(",");
			t = types.length;
			while (t--) {
				type = types[t].trim();
	...
	function doleOutWebmentions(webmentions) {
		var i = 0,
			...
			typeFilter = function (key) {
				// console.log( key, WebmentionIO.types[key], this.type );
				return WebmentionIO.types[key] === this.type;
			},
			typeFilterLoop;
		...
		while (i--) {
			...
			plural_type = Object.keys(WebmentionIO.types).filter(typeFilterLoop)[0];
			// console.log( queue_keys, plural_type );
		...
		while (i--) {
			...
			while (j--) {
				...
				if (incoming[queue_keys[i]].length > 0) {
					addWebmentionsToCollection(
						incoming[queue_keys[i]],
						webmention_receivers[queue_keys[i]][j],
					);
				}
			}
		}
	...
	function addWebmentionsToCollection(mentions, $webmention_collection) {
		// console.log( "adding", mentions, "to", $webmention_collection );
		...
		if (!$webmention_collection.template) {
			console.error(
				"No template found for this webmention group",
				$webmention_collection,
			);
			return;
		}
	...
	function removeDuplicates(webmentions) {
		...
		while (i--) {
			if (WebmentionIO.existing_webmentions.indexOf(webmentions[i].id) < 0) {
				unique_webmentions.push(webmentions[i]);
				WebmentionIO.existing_webmentions.push(webmentions[i].id);
			}
		}
	...
	function addMetadata(webmentions, config) {
		...
		while (i--) {
			...
			source = false;
			if (webmention.source.indexOf("/mastodon/")) {
				source = "mastodon";
			} else if (uri.indexOf("twitter.com/")) {
				source = "twitter";
			...
			if ("author" in webmention.data) {
				webmention_object.author = webmention.data.author;
			}

			type = webmention.activity.type;
			if (!type) {
			...
			if (
				webmention.activity.sentence_html &&
				type != "bookmark" &&
				type != "link" &&
				type != "post" &&
				type != "reply"
			...
			if (content) {
				...
				content = content.replace("href=", 'rel="nofollow" href=');
			}
					...
					return (
						...
						(u.parseSwitch = function () {
							...
							do {
								...
								(s.push(new e.Case(r.line, r.col, o, u)),
									(r = this.peekToken()));
...
(function (window, document) {
	...
	var targets = [],
		$redirects = document.querySelector(
			'meta[property="webmention:redirected_from"]',
		),
		redirects,
	...
	if (base_url.indexOf("localhost") > -1) {
		base_url = "https://www.aaron-gustafson.com";
	}
	...
	if ($redirects) {
		...
		redirects.forEach(function (value) {
			targets.push(value.indexOf("//") < 0 ? base_url + value : value);
		});
		redirects = false;
	}
	...
	$script.src =
		...
		"jsonp=window.WebmentionIO.processWebmentions&target[]=" +
		targets.join("&target[]=");
	document.querySelector("head").appendChild($script);
})(this, this.document);

(function (window, document, WebmentionIO) {
	"use strict";
	...
	var $webmention_link = document.querySelector('a[href="#webmentions"]'),
		$webmention_link_section = document.querySelector(
			'dd:has(a[href="#webmentions"])',
		),
		event_name = "WebmentionIO:update_counters";

	function updateCounts() {
		var count = document.querySelectorAll(".webmention").length;
		$webmention_link.innerText = count + " Webmentions";

		if ($webmention_link.parentNode.hidden) {
			$webmention_link.parentNode.hidden = false;
			$webmention_link_section.hidden = false;
			$webmention_link_section.previousSibling.hidden = false;
		}
	}

	if ($webmention_link) {
		WebmentionIO.counter_update_event = new Event(event_name);
		document.addEventListener(event_name, updateCounts, false);
	}
})(this, this.document, this.WebmentionIO);
